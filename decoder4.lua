...
-- big-endian 4-byte to integer
local function int(b)
    local n = 0
    for i=1,#b do n = 256*n + b:sub(i,i):byte() end
    return n
end
int = memo(int)
...
local function bits(b, w)
    ...
    if type(b)=="number" then
        ...
        for i=1,#b do s=s..bits(b:sub(i,i):byte(),8):reverse() end
    end
    return s
end
bits = memo(bits)
...
local function bint(s) return tonumber(s,2) or 0 end
bint = memo(bint)
...
local function byte_stream(raw)
    local pos = 0
    return {
        read = function(_,n) local c = raw:sub(pos+1,pos+n); pos+=n; return c end,
...
local function output_stream()
    local buf, p = {}, 0
    local S = {}
    function S.write(_,s) for i=1,#s do buf[#buf+1]=s:sub(i,i) end; p+=#s end
    function S.back_copy(_,d,l)
        local st = p-d+1; local seg={}
    ...
    for i,cd in ipairs(codes) do
        if #cd>PT_W then
            ...
            while done<#group do
                ...
                local tc,ta={},{}
                for _,p in ipairs(same) do tc[#tc+1]=p[1]:sub(PT_W+1); ta[#ta+1]=p[2] end
...
local function read_huffman(stream, lens, alpha)
    local tbl = prefix_table(lens, alpha)
    return function()
        local e = tbl[stream:read(PT_W,true)]
        stream:seek(-e.unused)
        while e.rest do
            local next_e = e.rest[stream:read(PT_W,true)]
            stream:seek(-next_e.unused)
            e = next_e
        end
        return e.value
    end
...
local function inflate(stream)
    ...
    while true do
        ...
        local t=bint(stream:read(2))
        if t~=2 then error("only dynamic Huffman") end
        local hlit=bint(stream:read(5))+257
        local hdist=bint(stream:read(5))+1
        local hclen=bint(stream:read(4))+4
        local cllens={}
        for i=1,hclen do cllens[#cllens+1]=bint(stream:read(3)) end
        local cldec=read_huffman(stream, cllens, CL_ORDER)
        local litlens=decode_lengths(stream, hlit, cldec)
        local distlens=decode_lengths(stream, hdist, cldec)
        local litdec=read_huffman(stream, litlens, {})
        local distdec=read_huffman(stream, distlens,{})
        while true do
            local sym=litdec(stream)
            if sym<256 then
                out:write(string.char(sym))
            elseif sym==256 then
                break
            else
                local length = (sym==285 and 258) or (3+sym-257+bint(stream:read((sym<4 or sym>29) and 0 or floor((sym-2)/2))))
                local dist = distdec(stream)
...
-- simple PNG loader with chunk loop
local function load_png(raw)
    ...
    while true do
        ...
        if ctype=="IHDR" then
            ...
            -- compression/filter/interlace in data:sub(11,13) ignored
    ...
    assert(idat~="","no IDAT")
    local rawde = inflate(bit_stream(idat,2))
    img.data = rawde
    return img
end

function png.load(data)
    local img = load_png(data)
    return img
end
        ...
        for y=0,img.height-1 do
            ...
            for x=0,img.width-1 do
                ...
                if img.color_type==0 then
                    local val = img.bit_depth==8 and row[x+1] or sample_bits(row,x,img.bit_depth)*255/(2^img.bit_depth-1)
