...
local function output_stream()
    ...
    function S.back_copy(_, dist, length)
        local start = pos - dist + 1
        local segment = {}
        for i = start, start + length - 1 do
            segment[#segment + 1] = buf[i]
        end
        local s = table.concat(segment)
...
local function prefix_table(codes, alpha)
    local pt = {}
    -- handle long codes
    local function handle_long(codes_list, symbols)
        for i, code in ipairs(codes_list) do
            if #code > PT_W then
                local group = {}
                for j = i, #codes_list do
                    group[#group + 1] = {codes_list[j], symbols[j]}
                end
                local done = 0
                while done < #group do
                    local prefix = group[done + 1][1]:sub(1, PT_W)
                    local same = {}
                    for _, pair in ipairs(group) do
                        if pair[1]:sub(1, PT_W) == prefix then
                            same[#same + 1] = pair
                        end
                    end
                    local tail_codes, tail_syms = {}, {}
                    for _, p in ipairs(same) do
                        tail_codes[#tail_codes + 1] = p[1]:sub(PT_W + 1)
                        tail_syms[#tail_syms + 1] = p[2]
                    end
                    pt[prefix] = {rest = prefix_table(tail_codes, tail_syms), unused = 0}
                    done = done + #same
                end
                break
            end
...
local function build_huffman(stream, lens, alpha)
    return function()
        ...
        local entry
        repeat
            entry = table[stream:read(PT_W, true)]
...
local function inflate(stream)
    ...
    while true do
        ...
        local dist_lens = decode_lengths(stream, hdist, cl_dec)

        local lit_dec = build_huffman(stream, codes(lit_lens), {})
        local dst_dec = build_huffman(stream, codes(dist_lens), {})
...
local function process_data(s, img)
    ...
    until s:read(4) ~= "IDAT"
    s:seek(-8, "cur")
    img.data = inflate(bit_stream(zb.raw(), 2))
end
            ...
            for y = 0, img.height - 1 do
                ...
                for i = 1, #raw do
                    ...
                    elseif filt == 4 then
                        -- paeth
                        local p = a + b - c
                        local pa, pb, pc = abs(p - l), abs(p - u), abs(p - c)
                ...
                for x = 0, img.width - 1 do
                    ...
                    elseif img.color_type == 2 then
                        local idx = x * 3
                        r, g, b, a = raw[idx + 1], raw[idx + 2], raw[idx + 3], 255
                    ...
                    elseif img.color_type == 4 then
                        local idx = x * 2
                        local g8, a8 = raw[idx + 1], raw[idx + 2]
                        r, g, b, a = g8, g8, g8, a8
                    elseif img.color_type == 6 then
                        local idx = x * 4
                        r, g, b, a = raw[idx + 1], raw[idx + 2], raw[idx + 3], raw[idx + 4]
                    end
