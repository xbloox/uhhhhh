...
local function int(bytes)                 -- big-endian bytes → int (or passthrough)
    if type(bytes) ~= "string" then return bytes end
    local n = 0
    for i = 1, #bytes do n = 256*n + bytes:byte(i) end
    return n
end

local function bits(n, w)                 -- number → bit-string (big-endian)
    local s = ""
    for _ = 1, w do
        s = (n % 2) .. s
        n = n // 2
...
local function byte_stream(raw)           -- minimal seek/read wrapper
    local pos = 0
    return {
        read = function(_, n)
            local s = raw:sub(pos+1, pos+n); pos = pos + n; return s
        end,
        seek = function(_, n, wh)
            if   wh == "beg" then pos = n
            elseif wh == "end" then pos = #raw
            else  pos = pos + n
            end
        end,
...
local function bit_stream(raw, off)
    ...
    return {
        ...
        seek     = function(_, n) pos = (n=="beg" and 0) or (n=="end" and 8*#raw) or (pos+n) end,
        is_empty = function() return pos >= 8*#raw end,
    }
end

local function output_stream()            -- buffer + back-copy (for inflate)
    local buf, p = {}, 0
    local O = {}
    ...
    function O.back_copy(_, dist, len)
        ...
        local seg   = {}
        for i = start, start+len-1 do seg[#seg+1] = buf[i] end
        local chunk = table.concat(seg)
        local rep   = math.ceil(len/#chunk)
        O.write(_, chunk:rep(rep):sub(1,len))
    end
    function O.raw() return table.concat(buf) end
    return O
end

-- ────────────────────────────── canonical-code utilities (NEW FIX) ──────────
-- convert *length array* → parallel arrays {codes},{symbols} sorted canonical
local function canonical_codes(lens)
    local pairs = {}
    for sym, L in ipairs(lens) do
        if L > 0 then pairs[#pairs+1] = {L, sym-1} end
    end
...
-- build prefix-lookup table (unchanged)
local PT_W = 8
...
-- build decoder from *length array* (fixed)
local function huffman_decoder(stream, lens)
    local codes, alpha = canonical_codes(lens)
    local base = prefix_table(codes, alpha)
    return function()
        local e = base[stream:read(PT_W,true)]
        stream:seek(-e.unused)
        return e.value
    end
end

-- ─────────────────────────────────────────────── inflate (dynamic blocks) ───
local function extra_bits(s) return (s<4 or s>29) and 0 or ((s-2)//2) end
local function inflate(stream)
    ...
    repeat
        ...
        local cl_lens = {}
        for i=1,hclen do cl_lens[i] = tonumber(stream:read(3),2) end
        local cl_dec  = huffman_decoder(stream, cl_lens)
        local lit_lens= {}
        local dst_lens= {}
        for i=1,hlit  do lit_lens[i] = cl_dec(stream) end
        for i=1,hdist do dst_lens[i] = cl_dec(stream) end
        local lit_dec = huffman_decoder(stream, lit_lens)
        local dst_dec = huffman_decoder(stream, dst_lens)
        while true do
...
local function load_png(raw)
    ...
    while true do
        ...
        elseif typ=="PLTE" then img.plte = {string.byte(data,1,-1)}
        elseif typ=="tRNS" then img.trns = data
        elseif typ=="IDAT" then idat = idat .. data
        elseif typ=="IEND" then break
        end
    end
...
function png.load(data) return load_png(data) end

local Http = request or http_request or (syn and syn.request)
function png.load_from_url(url)
    local r = Http({Url=url, Method="GET"}); assert(r.Success,"fetch failed")
        ...
        for y=0,img.height-1 do
            ...
            for x=0,img.width-1 do
                ...
                if img.color_type==0 then
                    local val = img.bit_depth==8 and row[x+1] or sample(row,x,img.bit_depth)*255/(2^img.bit_depth-1)
                    r,g,b,a = val,val,val,255
                elseif img.color_type==2 then
                    local i=x*3; r,g,b,a = row[i+1],row[i+2],row[i+3],255
                ...
                elseif img.color_type==4 then
                    local i=x*2; local g8,a8=row[i+1],row[i+2]; r,g,b,a=g8,g8,g8,a8
                elseif img.color_type==6 then
                    local i=x*4; r,g,b,a=row[i+1],row[i+2],row[i+3],row[i+4]
                end
